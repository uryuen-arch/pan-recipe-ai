...
const FALLBACK_PROFILES = [
  { id: 'f1', type: '菓子パン生地', dough_type: 'rich', texture: 'ふんわり', difficulty: '簡単', water: 0, milk: 65, sugar: 10, salt: 2, yeast: 1.5, butter: 10, egg: 15, is_active: true, description: 'リッチな味わいの基本生地' },
  { id: 'f2', type: 'シンプル生地', dough_type: 'simple', texture: 'ふんわり', difficulty: '超簡単', water: 65, milk: 0, sugar: 5, salt: 2, yeast: 1.5, butter: 0, egg: 0, is_active: true, description: 'どんな具材にも合う万能生地' }
];
...
export async function POST(request) {
  try {
    ...
    console.log(`DB Counts: Profiles=${profiles.length}, Components=${components.length}, Breads=${breads.length}`);

    // DBが空（または取得失敗）の場合のフォールバック
    ...
    console.log(`Match Counts: MatchedBreads=${matchedBreadsList.length}, MatchedProfiles=${matchedProfiles.length}`);

    // 3. 提案候補の選択（上位3件を確実に確保）
    ...
    // まだ3件に満たない場合（非常に稀）はフォールバックから補充
    if (finalSelection.length < 3) {
      finalSelection = [...finalSelection, ...FALLBACK_PROFILES.slice(0, 3 - finalSelection.length).map(p => ({ profile: p, type: "base", category: 'almost', missing: [] }))];
    }
    ...
    const fillingsText = `\n【具材指示】
1. 特徴づける具材を選別してください。
2. ペースト状素材（バナナ等）は「捏ね」で練り込む提案を優先。
3. 全ての具材に「timing」と「inst」を出力。
- timing：混ぜる/捏ね/一次発酵/成形/焼成前/仕上げ
- inst：動作（15文字以内）`;
